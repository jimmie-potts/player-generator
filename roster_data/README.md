...
The three CSVs share stable generated `playerId` values. The manifest identifies player data
contract version 1 and the roster profile, the reference-package content hash, formula version and
document hash, seed, semantic configuration hash, row counts, per-file hashes, and aggregate content
hash.
