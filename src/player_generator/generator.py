...
from player_generator.names import TEAMS
from player_generator.schema import POSITION_GROUPS, RATING_FIELDS, TIER_ORDER
from player_generator.util import clamp_int
...
def _validate_targets(config: dict[str, Any]) -> int:
    ...
    if int(generation["team_count"]) > len(TEAMS):
        raise ValueError(f"Only {len(TEAMS)} fictional team definitions are available.")
...
def _unique_name(
    ...
    for _ in range(500):
        ...
        if normalized not in forbidden_names and normalized not in used_names:
            ...
            return first, last
    raise RuntimeError("Unable to generate a unique fictional player name.")
