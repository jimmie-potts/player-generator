...
The application accepts only the version 1 reference profile and validates the complete manifest,
files, hashes, row counts, contract identity, relationships, and formula requirements before
sampling. Published reference attributes are validated, but roster selection and output evaluate
the requested formula from statistical inputs. The application mutates primitive totals, derives
dependent statistics, calculates attributes through `player_attribute_engine`, validates the three
version 1 roster-profile CSVs, scans for reference identity leakage, and replaces the destination
